...
qtl2_cim <- read.delim("outputs/qtl2_cim.tsv") %>%
  mutate(Position = round(as.numeric(sapply(strsplit(Pos, "_"), "[", 2))/1e6, 1),
         chr = gsub("\\..", "", chr)) %>%
  dplyr::rename(Cross_ID = cross, marker = Pos, Chromosome = chr) %>%
...
gwas <- merge(gwas_results, gece, by = c("marker", "trait")) %>%
  dplyr::rename(AA = adj_pheno_0, BB = adj_pheno_2)
...
peak_summary3 <- mutate(peak_summary3, cross_label = factor(cross_label, levels = c( "GWAS", rev(parentage_order[[1]])))) %>%
  dplyr::select(trait, Chromosome, Position, LOD, AA, cross_label) %>%
  mutate(LOD_binned = cut(LOD, 
                          breaks = c(0, 5, 10, Inf),
                          labels = c("2.5-5", "5-10", ">10")))
...
wdrhd <- peak_summary3 %>%
  select(Chromosome, cross_label, trait, AA, LOD_binned) %>%
...
make_triangle_coords_binned <- function(x, y, triangle_type, lod_binned) {
  # Map LOD_binned to fixed triangle sizes
  size <- case_when(
    lod_binned == "<5"   ~ 0.2,
    lod_binned == "5-10" ~ 0.3,
    lod_binned == ">10"  ~ 0.45,
    TRUE ~ 0.2  # default fallback if unknown
  )
...
triangles <- wdrhd %>%
  rowwise() %>%
  mutate(coords = list(make_triangle_coords_binned(x, y, triangle, LOD_binned))) %>%
  unnest(coords) %>%
  ungroup()

lod_legend_df <- tibble(
  x = NA, y = NA,  # not shown
  LOD_binned = factor(c("<5", "5-10", "10+"), levels = c("<5", "5-10", "10+"))
)
