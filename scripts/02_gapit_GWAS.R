...
for (trait in colnames(blues)[-c(1)]) {
  ...
  gtable$trait <- trait
  gtable$model <- "MLM"
  if (exists("MLM_table") & nrow(gtable) > 1) {MLM_table  <- rbind(MLM_table , gtable)} else {MLM_table  <- gtable}
}
...
for (trait in colnames(blues)[-c(1)]) {
  ...
  gtable$trait <- trait
  gtable$model <- "BLINK"
  if (exists("BLINK_table") & nrow(gtable) > 1) {BLINK_table  <- rbind(BLINK_table , gtable)} else {BLINK_table  <- gtable}
}
...
for (trait in colnames(blues)[-c(1)]) {
  ...
  if (exists("FarmCPU_table") & nrow(gtable) > 1) {FarmCPU_table  <- rbind(FarmCPU_table , gtable)} else {FarmCPU_table  <- gtable}
}
